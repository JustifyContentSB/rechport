...
import { paths, device, names } from "../config";

const { markers: path } = paths;

const isDesktop = device.platform.type === "desktop";
...
export default class extends Component {
    ...
    getTileContainer = (map) => {
        ...
        for (let k in layer) {
            if (layer.hasOwnProperty(k)) {
                if (
                    layer[k] instanceof CanvasContainer ||
                    layer[k] instanceof DomContainer
                ) {
                    return layer[k];
                }
            }
    ...
    setOffset = (el, map, offset) => {
        ...
        const geoCenter = map.options
            .get("projection")
            .fromGlobalPixels(updatedCenter, zoom);
    ...
    render = (el) => {
        ...
        if ($(el).closest(".modal").length) {
            options.controls = options.controls.filter(
                (control) => control !== "fullscreenControl"
            );
        }
